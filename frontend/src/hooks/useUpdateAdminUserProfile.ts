...
// Named distinctly from useAdminUserProfile(id) (the GET hook) — read/write split, matching
// every other resource in this SDK (docs/CONTRACT.md §10 item 10).
export function useUpdateAdminUserProfile(id: number) {
  ...
  return useMutation({
    mutationFn: (data: UpdateAdminProfileInput) => manager.updateUserProfile(id, data),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: dynamicUserAdminKeys.userProfile(id) });
    },
  });
}
